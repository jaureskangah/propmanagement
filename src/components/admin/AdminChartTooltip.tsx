...
export const AdminChartTooltip = ({ active, payload, label }: AdminChartTooltipProps) => {
  const { t } = useLocale();
          ...
          <div className="space-y-1 text-xs">
            {payload.map((entry, index) => {
              ...
              switch (entry.dataKey) {
                case 'total_users':
                  translatedLabel = t('totalUsers');
                  break;
                case 'active_users':
                  translatedLabel = t('activeUsers');
                  break;
                case 'total_revenue':
                  translatedLabel = t('totalRevenue');
                  break;
                case 'total_properties':
                  translatedLabel = t('properties');
                  break;
                case 'total_tenants':
                  translatedLabel = t('totalTenants');
                  break;
                default:
                  translatedLabel = entry.name;
              }
              
              const value = entry.dataKey === 'total_revenue' 
